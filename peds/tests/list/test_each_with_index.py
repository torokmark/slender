...
class TestEachWithIndex(TestCase):
    ...
    def test_each_with_index_if_param_is_none_and_func_is_not_none(self):
        e = List([1, 2, 3, 4])
        it = e.each_with_index(lambda index, item: [index, item * index] )
    ...
    def test_each_with_index_if_param_is_const_and_func_is_not_none(self):
        e = List([1, 2, 3, 4])
        it = e.each_with_index(2, lambda index, item: [index, item * index] )
        expect(next(it)).to(equal([2, 6]))
        expect(next(it)).to(equal([3, 12]))

    def test_each_with_index_if_param_is_negative(self):
        e = List([1, 2, 3, 4])
        it = e.each_with_index(-1)
        expect(lambda: next(it)).to(raise_error(TypeError))

    def test_each_with_index_if_param_is_different(self):
        e = List([1, 2, 3, 4])
        it = e.each_with_index('...')
        expect(lambda: next(it)).to(raise_error(TypeError))
